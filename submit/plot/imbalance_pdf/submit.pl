...
for($W = 0.1; $W <= 10; $W += 0.1)
{
	...
	mkdir "$data_path/$prefix/Nc_${Nc}/dt_${diss_type}/dp_${diss_phase_str}/et_${energy_type}/bc_${periodic_bc}/W_${W_str}/U_${U_str}/J_${J_str}/g_${g_str}/imbalance_pdf";

	$key = "$data_path/$prefix/Nc_${Nc}/dt_${diss_type}/dp_${diss_phase_str}/et_${energy_type}/bc_${periodic_bc}/W_${W_str}/U_${U_str}/J_${J_str}/g_${g_str}/imbalance_pdf";
		
	open( WF,">$key/config.txt");
	print WF "$W\n";
	close WF;
		
	$test_file = sprintf("imbalance_pdf_Nc(%d)_dt(%d)_dp(%0.4f)_et(%d)_bc(%d)_W(%0.4f)_U(%0.4f)_J(%0.4f)_g(%0.4f)_seed(var).txt", $Nc, $diss_type, $diss_phase, $energy_type, $periodic_bc, $W, $U, $J, $g);
