...
for($alpha = 0; $alpha <= pi; $alpha += pi/10)
{
	...
	{	
		...
		$test_file = sprintf('energies_Nc(%d)_dt(%d)_alpha(%0.4f)_et(%d)_W(%0.4f)_U(%0.4f)_J(%0.4f)_g(%0.4f)_max_num_seeds(%d)_seed(%d).txt', $Nc, $dt, $alpha, $et, $W, $U, $J, $g, $max_num_seeds, $seed);
