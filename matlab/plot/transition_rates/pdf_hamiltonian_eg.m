...
for i = 1:num_intervals_eg
	...
	count_in_intervals(i) = count_in_intervals(i) / (size_total * 2 / num_intervals_eg);
end


figure;
hLine = plot(intervals_eg, count_in_intervals, '-o', 'LineWidth', 2);
set(gca, 'FontSize', 30);
xlabel('$\epsilon$', 'Interpreter', 'latex');
set(gca, 'FontSize', 30);
ylabel('$PDF(\epsilon)$', 'Interpreter', 'latex');

savefig(sprintf('pdf_hamiltonian_eg_Nc(%d)_dt(%d)_alpha(%0.4f)_et(%d)_W(%0.4f)_U(%0.4f)_J(%0.4f)_g(%0.4f)_max_num_seeds(%d)_ss(%d)_sn(%d).fig', ...
