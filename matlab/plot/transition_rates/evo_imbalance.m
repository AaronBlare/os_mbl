...
for W = W_start : W_start + (num_Ws - 1)
    ...
    for seed = seed_start : seed_start + (num_seeds - 1)
        ...
        local_imb = local_imb + abs(data(2));
    end
    
    local_imb = local_imb / num_seeds;
    
    abs_imb(W - W_start + 1) = local_imb;
    
end

figure;
hLine = plot(Ws, abs_imb, '-o', 'LineWidth', 2);
set(gca, 'FontSize', 30);
xlabel('$W$', 'Interpreter', 'latex');
set(gca, 'FontSize', 30);
ylabel('$<|I|>$', 'Interpreter', 'latex');

savefig(sprintf('evo_imbalance_Nc(%d)_dt(%d)_alpha(%0.4f)_et(%d)_W(%0.4f)_U(%0.4f)_J(%0.4f)_g(%0.4f)_max_num_seeds(%d)_ss(%d)_sn(%d).fig', ...
