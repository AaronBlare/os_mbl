...
ipr = zeros(num_log_dumps, 1);

for seed = seed_start : seed_start + (num_seeds - 1)

	seed = seed
     
    file_name = sprintf('%s/seed_%d/ipr_and_Nc(%d)_dt(%d)_W(%0.2f)_U(%0.2f)_J(%0.2f)_gamma(%0.2f)_it(%d)_is(%d)_seed%d.txt', local_path, seed, Nc, diss_type, W, U, J, g, init_state_type, init_state_id, seed);
    local_data = importdata(file_name);
	local_data = local_data(1+1 : num_log_dumps+1);
    
    for dump_id = 1:num_log_dumps
        ipr(dump_id) = ipr(dump_id) + local_data(dump_id) / num_seeds;
    end
    
end

hLine = plot(log_times * g, coeff * ipr);
